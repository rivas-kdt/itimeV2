...
export default function EditWorkOrdersPage() {
    ...
    const { recordsInfo, recordsTotal, recordsInfoLoading, recordsInfoError, refetch } = useWorkOrderHooks(workOrderId, workCodeId, constructionItemId, othersId)
...
const getInspectionsForDate = (date?: Date) => {
    ...
    const itemListValues = useMemo(() => itemList.map((item) => item.value), [itemList]);
    const workCodeListValues = useMemo(() => workCodeList.map((item) => item.value), [workCodeList]);
    const othersListValues = useMemo(() => othersList.map((item) => item.value), [othersList]);
    ...
    const handleSaveEdit = async () => {
        ...
        const inspectionIds = recordsInfo?.map(record => record.id || record.inspection_id).filter(Boolean) || [];
                    ...
                    {workOrder && (
                        <div className="space-y-3">
                            ...
                            {isEditingTime && (
                                <>
                                    {(() => {
                                        const inspectionsOnDate = getInspectionsForDate(selectedDate);
