...
import { TimeStepper, TimeValue } from "@/features/workorders/components/timeSelector"
...
export default function EditWorkOrdersPage() {
  ...
  // Type guard to ensure data is WorkOrder or null
  const workOrder = useMemo(() => {
    if (!recordsInfo || !Array.isArray(recordsInfo) || recordsInfo.length === 0) return null;
  ...
  return (
    <div className="bg-background h-full overflow-y-scroll text-black">
      ...
      <div className="pb-8">
        ...
        {recordsInfoError && !recordsInfoLoading && <ErrorAlert message={recordsInfoError} onRetry={refetch} />}

        {!recordsInfoLoading && !recordsInfoError && workOrder && (
          <>
            {/* Work Order Details */}
            {isEditing && formData ? (
          ...
          {workOrder && (
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between px-3 py-2 bg-primary-op-2 rounded-md">
                <span className="font-bold">Total Inspection Time:</span>
                <span className="font-semibold">
                  {getTotalDurationForDate(selectedDate).hours}h {getTotalDurationForDate(selectedDate).minutes}m
                </span>
              </div>
